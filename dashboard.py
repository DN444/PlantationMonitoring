...
def load_finetuned_resnet50(weights_path):
...
def show_dashboard():
    ...
    if "mask_generator" not in st.session_state:
        with st.spinner("🔄 Loading SAM model (please wait 20–60s)..."):
            st.session_state.mask_generator = load_sam(r"plantation-monitoring\Fine Tuned SAM\fine_tuned_sam_vit_b_final.pth")
        st.success("✅ SAM model loaded successfully!")

    if "resnet_model" not in st.session_state:
        with st.spinner("🧠 Loading ResNet50 model..."):
            st.session_state.resnet_model = load_finetuned_resnet50(r"plantation-monitoring\models\best_resnet50.pth")
